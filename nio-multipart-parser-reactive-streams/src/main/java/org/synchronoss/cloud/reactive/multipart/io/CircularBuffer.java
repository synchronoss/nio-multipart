...
public class CircularBuffer {
    ...
    public static final byte[] EMPTY_READ = new byte[0];
    ...
    public byte[] readAll() {
        ...
        return readChunk(availableReadLength);
    }
     ...
     * @return The bytes read.
     */
    public byte[] readChunk(final int chunkSize) {
        ...
        final byte[] read = new byte[chunkSize];;
        if (startValidDataIndex + chunkSize > buffer.length){
            int firstChunkLength = buffer.length - startValidDataIndex;
            System.arraycopy(buffer, startValidDataIndex, read, 0, firstChunkLength);
            //outputStream.write(buffer, startValidDataIndex, firstChunkLength);
            System.arraycopy(buffer, 0, read, firstChunkLength +1, chunkSize - firstChunkLength);
            //outputStream.write(buffer, 0, chunkSize - firstChunkLength);
        }else{
            System.arraycopy(buffer, 0, read, startValidDataIndex, chunkSize);
